        ...
        if type_ is not None:
            ...
            # Если тип является списком, проверить тип элементов
                ...
                if not isinstance(value, list):
                    raise InvalidTypeError(f"argument must be of type list[{element_type.__name__}]")
                for item in value:
                    if not isinstance(item, element_type):
                        raise InvalidTypeError(f"all elements must be of type {element_type.__name__}")
